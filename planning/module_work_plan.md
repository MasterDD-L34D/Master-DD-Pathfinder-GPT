...
| ENC-OBS-02 | Osservazione | Tracciare la pipeline completa (setup → auto-bilanciamento → QA → export VTT/MD/PDF) con CTA obbligatorie verso i comandi chiave e auto-validazione prima dell’export. | S2 (Minor) | - Descrizione pipeline e CTA registrate come definizione di pronto.<br>- Verifica che ogni fase richiami i comandi citati nei gate QA.<br>- Checkpoint 2025-12-12 approva la checklist. | Piano: L25-L27. Codice: `src/modules/Encounter_Designer.txt` (pipeline e CTA QA/export).【F:src/modules/Encounter_Designer.txt†L486-L523】【F:src/modules/Encounter_Designer.txt†L400-L419】 | To Do |
| ENC-ERR-01 | Errore | Conservare evidenza che non risultano errori bloccanti su CR/QA dopo l’allineamento al helper clampato. | S3 (Info) | - Nota “nessun errore bloccante” collegata al test CR/QA.<br>- QA ripete il test clampato e allega esito nel tracker.<br>- Validato entro il checkpoint 2025-12-12. | Piano: L26-L27. Codice: `src/modules/Encounter_Designer.txt` (calcolo CR/QA clampato).【F:src/modules/Encounter_Designer.txt†L293-L314】【F:src/modules/Encounter_Designer.txt†L777-L788】 | To Do |

#### sigilli_runner_module — Owner: Fabio Marchetti — Checkpoint: 2025-12-26
| Story ID | Deriva da | Descrizione | Severità | Acceptance Criteria | Tracciabilità (piano → codice) | Stato |
| --- | --- | --- | --- | --- | --- | --- |
| SIG-OBS-01 | Osservazione | Evidenziare la finestra di attivazione del raro solo da indice 14 con stato di default per evitare percezione di malfunzionamento iniziale. | S2 (Minor) | - Nota di comportamento “rare da indice 14” visibile nel tracker.<br>- QA riproduce l’attivazione a indice 14 e allega evidenza.<br>- Checkpoint 2025-12-26 valida la nota. | Piano: L221-L222. Codice: `src/modules/sigilli_runner_module.txt` (logica raro).【F:src/modules/sigilli_runner_module.txt†L116-L148】 | To Do |
| SIG-OBS-02 | Osservazione | Documentare che il portale viene aggiunto anche quando nessun sigillo è stato assegnato, garantendo almeno un elemento in `seals`. | S3 (Info) | - Nota portale sempre presente registrata nel tracker.<br>- QA verifica risposta con `seals` popolato solo dal portale.<br>- Checkpoint 2025-12-26 chiude la verifica. | Piano: L222-L223. Codice: `src/modules/sigilli_runner_module.txt` (aggiunta portale).【F:src/modules/sigilli_runner_module.txt†L144-L154】 | To Do |
| SIG-OBS-03 | Osservazione | Registrare che il report copre API, metadati, modello dati, flow/CTA, errori simulati e fix applicati senza ulteriori lacune note. | S3 (Info) | - Nota di completezza report archiviata nel tracker.<br>- QA allega riferimento al set di test eseguiti.<br>- Checkpoint 2025-12-26 approva la nota. | Piano: L223-L224. Codice: `src/modules/sigilli_runner_module.txt` (model e flow CTA).【F:src/modules/sigilli_runner_module.txt†L106-L159】 | To Do |
| SIG-ERR-01 | Errore | API key mancante su `/modules*` restituisce `401 Invalid or missing API key`. | S1 (Major) | - Test QA allegato con richiesta senza API key e risposta 401.<br>- Nota collegata ai log/trace di sicurezza.<br>- Validazione al checkpoint 2025-12-26. | Piano: L225-L226. Codice: `tests`/`TestClient` esito 401.【fc8c1a†L3-L12】 | To Do |
| SIG-ERR-02 | Errore | `/modules/bogus.txt` ritorna `404 Module not found`. | S2 (Minor) | - Test QA allegato per modulo inesistente con 404.<br>- Ticket include output grezzo del test.<br>- Checkpoint 2025-12-26 conferma. | Piano: L226-L227. Codice: API lookup mancante modulo.【5c31d3†L9-L10】 | To Do |
| SIG-ERR-03 | Errore | Con `ALLOW_MODULE_DUMP=false` il dump viene troncato con header coerente per evitare leak completi. | S1 (Major) | - Test QA allegato con header di troncamento.<br>- Verifica che il marker di dump limitato sia presente.<br>- Validato al checkpoint 2025-12-26. | Piano: L227-L228. Codice: `src/modules/sigilli_runner_module.txt` (troncamento e header).【F:src/modules/sigilli_runner_module.txt†L148-L159】【5c31d3†L11-L18】 | To Do |
| SIG-ERR-04 | Errore | Nessun errore bloccante dopo integrazione di `code_ok` e tagging MDA/CTA nei sigilli. | S3 (Info) | - Nota “nessun errore bloccante” collegata ai test post `code_ok`.<br>- QA allega log di esecuzione con tagging MDA/CTA.<br>- Checkpoint 2025-12-26 registra l’evidenza. | Piano: L228. Codice: `src/modules/sigilli_runner_module.txt` (tagging MDA/CTA).【F:src/modules/sigilli_runner_module.txt†L106-L144】 | To Do |
...
| BAS-OBS-02 | Osservazione | Dipendenza unica: router hard-gate ai moduli core (binding file locale) con preload obbligatorio `preload_all_modules` protetto da `x-api-key`. | S3 (Info) | - Sezione dipendenze aggiornata con elenco moduli core e link al codice router/preload.<br>- Verifica preload con API key valida e flag `runtime.preload_done` attivo.<br>- Convalida nel checkpoint 2025-12-19. | Piano: L84-L91. Codice: `src/modules/base_profile.txt` (binding moduli core e preload).【F:src/modules/base_profile.txt†L107-L146】【F:src/modules/base_profile.txt†L252-L307】 | In Review |
| BAS-CHK-19 | Checkpoint | Checklist readiness 2025-12-19 (API key, preload, moduli core disponibili). | S2 (Minor) | - API key e endpoint preload verificati (200 / 401 previsto su assenza key).<br>- Preload eseguito (flag runtime, warmup e decorator attivi).<br>- Binding ai moduli core disponibili su disco prima di avviare i fix P1. | Piano: L88-L91. Codice: `tests/test_app.py` (health/doc e dump policy).【F:tests/test_app.py†L282-L314】【F:tests/test_app.py†L547-L591】 | To Do |
...
| base_profile | Hard-gate moduli core + preload protetto | BAS-OBS-02 | S3 | Andrea Rizzi | In Review |
| base_profile | Checkpoint readiness 2025-12-19 | BAS-CHK-19 | S2 | Andrea Rizzi | To Do |
