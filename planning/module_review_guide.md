...
- Normalizza i report prima del piano (e comunque prima della revisione manuale) con `python tools/refresh_module_reports.py --write`: lo script legge la sequenza moduli dalla guida, tocca tutti i file in `reports/module_tests/` e inserisce le sezioni obbligatorie (Ambiente, Esiti API, Metadati, Comandi/Flow, QA, Osservazioni, Errori, Miglioramenti, Fix necessari) con bullet placeholder. I blocchi Metadati/Comandi vengono precompilati con nome, versione, trigger e comandi estratti dal sorgente modulo. Usa `--check` per validare la presenza di tutte le sezioni (exit 1 in caso di heading mancante o vuoto) prima di lanciare `generate_module_plan`.
