...
- Prima di lanciare il generatore del piano (e comunque prima della revisione manuale), esegui lo script di normalizzazione dei report: `python tools/refresh_module_reports.py --write` per applicare il template e `python tools/refresh_module_reports.py --check` per validare che tutte le sezioni standard siano presenti. Il comando va usato prima di aprire i file in lettura per assicurare placeholder e intestazioni completi, così il piano riflette la checklist aggiornata.
