...
- Prima di lanciare il generatore del piano (e comunque prima della revisione manuale), esegui lo script di normalizzazione dei report. Usa `python tools/refresh_module_reports.py --write` per applicare il template standard (metadati/comandi precompilati) e `python tools/refresh_module_reports.py --check` per validare che tutte le sezioni obbligatorie siano presenti. Fai girare lo script prima del generatore (`generate_module_plan`) così il piano viene costruito su report già allineati alla checklist.
