...
| `data_background_traits.xml` | 1.569 | Name, Type, Description, ClassSkill(Choice), Skill(Bonus), Fort/Reflex/Will, Initiative, rAlign/rClass/rFaction/rRace/rReligion, Source, Ref | **Importato (C1 2026-08-08)**: `pathbuilder-background-traits.json` — 1.569 tratti nome+categoria (mappa Type→categoria esplicita, cross-check Ref; 2 nomi duplicati dichiarati; MAI Description/Ref) |
...
| `data_armor_magic.xml` | 68 | Categories, Effect | **Importato (Fase A 2026-08-08)**: `pathbuilder-magic-qualities.json` — 68 qualità magiche per armature/scudi/accessori (nome + categorie di applicabilità; mappe codici riusate da D4; MAI description — non esiste nel dato — MAI bonus/prezzi inventati) |
...
| `data_weapon_effects.xml` | 97 | Name, Categories, Damage | **Importato (Fase A 2026-08-08)**: `pathbuilder-magic-qualities.json` — 97 qualità magiche per armi (13 con `<Damage>` stringa grezza strutturata, MAI parsato a effetto meccanico). Consumatore: nessuno ancora (dichiarato) — candidato legality equip D6 |
