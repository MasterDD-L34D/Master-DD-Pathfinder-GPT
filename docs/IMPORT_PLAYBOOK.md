...
**Residui C2 task 2 (2026-07-26, stessa giornata)**: `talents.json` 1041→**1559** OGL (+518) + 5 local. **magus arcana** 64 (`MagusArcana.aspx`, parser tabellare standard). **Blocchi-scelta a pagine dettaglio** (indice → link `MysteryDisplay`/`BloodlineDisplay`/`CavalierOrders?ItemName=`; 123 pagine, UN solo span `LabelName` con sezioni a bold): **mystery** 34 (description = prosa Class Skills + Bonus Spells; `mechanics_extra {class_skills, bonus_spells[{name,level}], final_revelation?}`; sezione **Deities MAI letta** — nomi di divinità = PI) + **revelation** 331 (entry inline `<i>Nome (kind)</i>:` tra i bold "Revelations" e "Final Revelation", category = mystery; la stessa revelation offerta da più mystery — 29 nomi condivisi, es. Iron Skin battle/metal — è un'istanza selezionabile distinta: dedup e source_id estesi con la category); **bloodline** 51 (description = flavor; `mechanics_extra {class_skill, bonus_spells, bonus_feats, arcana, powers[nome+kind]}` — i powers sono auto-conferiti, NOMI soltanto: testi via reference_url, assenza onesta); **order** 38 (`CavalierOrders.aspx?ItemName=`, pool condiviso cavalier/samurai come skald/rage; description = flavor; `mechanics_extra.skills` — 3 formulazioni reali: "adds X to her list of class skills" / "adds X to his class skills" / "gains X as class skills"). Edicts/challenge/abilità ordini e testi powers NON serializzati (blocchi lunghi non selezionabili, via reference_url). **PI**: 5 revelation con nome di divinità ("X's Boon": Abadar, Asmodeus, Iomedae, Irori, Torag) → `talents_local.json` (kind `talents_local` attivato nel manifest). 2 description senza punto finale (Dispelling Strike, Draconic Resistance) = troncamenti della fonte stessa (verificato su HTML grezzo, assenza onesta). Test: +3 fixture dettaglio + varianti skills → 15 test.
